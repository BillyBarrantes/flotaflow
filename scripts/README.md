...
`flb-package-linker` manages local FlotaFlow package links for extension development. It updates the Console npm manifest, API Composer repositories, and Console pnpm workspace settings so linked extension packages and shared Ember packages resolve from `packages/*`.
...
The linker only treats a package as a FlotaFlow extension when either:

- its `package.json` has `flotaflow-extension` in `keywords`
- it has an `extension.json` manifest

The only non-extension packages supported for shared dependency linking are `@flotaflow/ember-core`, `@flotaflow/ember-ui`, `@flotaflow/fleetops-data`, and the backend-only `flotaflow/core-api`.
...
Check for missing local symlinks or duplicate FlotaFlow package versions in `console/pnpm-lock.yaml`:
...
- `console/.npmrc`
- `api/composer.json`
- `.flotaflow-dev-links.json`

The `.flotaflow-dev-links.json` file stores original dependency ranges so links can be reversed. It is ignored by git.
...
`docker-install.sh` is the interactive FlotaFlow Docker setup wizard. It checks required local tools, asks for core environment settings, generates local configuration, and guides Docker Compose setup.
...
The script expects Docker, Docker Compose v2, git, and OpenSSL to be available. It warns when common FlotaFlow ports are already in use but does not treat that as a hard failure.
